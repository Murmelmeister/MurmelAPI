...
public final class UserParentProviderImpl implements UserParentProvider {
    ...
    public UserParent add(int userId, int parentId, long duration, int createdBy) {
        ...
        int row = database.update(insertSql, userId, parentId, expiresAt, createdBy);
        ...
        LocalDateTime createdAt = database.query(selectSql, null, resultSet ->
                resultSet.getTimestamp("created_at").toLocalDateTime(), userId, parentId);
    ...
    public int remove(int userId, int parentId) {
        ...
        int row = database.update(sql, userId, parentId);
        if (row < 1) return 0;
    ...
    public UserParent update(int userId, int parentId, long duration, int changedBy) {
        ...
        int row = database.update(updateSql, expiresAt, changedBy, userId, parentId);
        if (row < 1) return null;

        String selectSql = "SELECT changed_at FROM " + TABLE_NAME + " WHERE user_id = ? AND parent_id = ?";
        LocalDateTime changedAt = database.query(selectSql, null, resultSet ->
                resultSet.getTimestamp("changed_at").toLocalDateTime(), userId, parentId);
